...
* TODO test skipping comment line in infile.go
    
* WAIT checkpoint system - resume from incomplete run
  - need to test on thirds and fourths, ideally on eland
  - seconds look good
...
** DONE molpro job information
   ...
   - charge
   - spin
** WAIT mopac job information - untested, waiting on eland
   - method
   - charge
** WAIT follow Psi4 input style
   - not doing this for now
    
* WAIT add mopac i/o - waiting on eland
** DONE read inputs:
*** DONE equilibrium geometry - same as molpro, xyz file
*** DONE parameters
    - just link to params file - params.dat hard coded
** WAIT write input files - need to test
